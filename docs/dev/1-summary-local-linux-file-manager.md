...
- 完成状态：部分完成。已完成最小只读浏览骨架和当前目录树文件名正则搜索；写操作、外部二进制适配、选择/多选和真实图形会话 smoke test 待后续。
...
- 代码逻辑改动：
  ...
  - 地址栏回车时只有绝对路径按路径跳转；所有非绝对路径输入都作为正则表达式在当前目录树搜索文件/目录名，搜索结果在主区域列出，副标题显示相对路径；隐藏文件开关会重新扫描或重新执行当前搜索。
  - 目录扫描和文件名正则搜索改为 iced `Task::perform` 后台任务；GUI 使用自增请求 ID 丢弃过期结果，避免旧后台结果覆盖新导航/搜索状态。
...
- 影响的使用场景：可以从当前工作目录启动 GUI，以图形文件管理器布局浏览本地目录，并通过地址栏正则搜索当前目录树文件名。
...
- 执行验证：
  - `cargo fmt --check`：通过。
  - `cargo test -p filesystem-core`：通过，8 个单元测试。
...
| 高级工程师 | 实现保持小范围；窗口操作复用 iced 原生 task，目录扫描和搜索通过后台任务执行；新增 `regex` 依赖用于用户要求的正则匹配；错误路径能向 UI 展示；自动化验证覆盖当前只读 API。未覆盖真实开窗、权限错误、大目录性能和后台任务压力，是下一阶段风险。 |
