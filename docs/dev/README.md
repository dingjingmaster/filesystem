...
| 48 | 2026-09-16 | L2 | fix | [48-fix-yunbox-wps-spreadsheets-open.md](48-fix-yunbox-wps-spreadsheets-open.md) | 已完成 | 修复 Kylin 云盒中 `.et` 经 `wpsoffice /prometheus` 打开报错的问题，云盒 WPS Spreadsheets 改为优先直启 `office6/et`，Writer 保留 Prometheus 兼容入口。 |
| 49 | 2026-09-18 | L2 | fix | [49-fix-yunbox-wps-prometheus-open.md](49-fix-yunbox-wps-prometheus-open.md) | 已完成 | 修复 65 云盒内点击 `.et` 无反应：现场 `strace` 确认 `office6/et` 内部执行 `/et /prometheus` 失败后成功退出，云盒 WPS Writer/表格/演示统一恢复优先 `wpsoffice /prometheus`，并保留 `.desktop` fallback。 |
| 50 | 2026-09-20 | L2 | fix | [50-fix-yunbox-wps-user-env.md](50-fix-yunbox-wps-user-env.md) | 已完成 | 修复云盒/pkexec/root 启动链向 WPS 传播 root 用户环境导致文档锁定或初始化异常的问题，云盒外部应用启动前优先按 `PKEXEC_UID` 反查真实桌面用户并设置 `USER/LOGNAME/USERNAME`，42/65 实机验证 WPS 文字、表格、演示均可打开。 |
