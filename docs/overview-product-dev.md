...
- 核心数据结构：
  ...
  - `TemplateFile`：空白菜单“新建模板文件”子菜单使用的模板文件显示名和源路径；显示名来自模板路径 `file_stem()`，因此菜单不显示最后一个扩展名，创建时仍使用源路径原始文件名。
