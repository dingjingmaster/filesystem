...
- 启动配置规则：GUI 启动时通过 `current_exe()` 定位可执行文件同级 `filesystem.ini`，只解析 `name` 和 `terminal` 两个非空键；`name` 写入 `RuntimeConfig` 后覆盖左侧标题栏显示，`terminal` 写入 `RuntimeConfig` 后作为右键“在终端打开”的优先终端路径；配置文件不存在、读取失败或键为空时使用默认值。
...
- 核心数据结构：
  ...
  - `RuntimeConfig`：启动时读取的本地运行时配置，当前包含左侧标题栏名称和可选终端路径。
...
- 配置文件/参数：启动时读取可执行文件同级 `filesystem.ini`；支持 INI 键 `name=xxx` 覆盖左侧标题栏显示，`terminal=xxx` 配置右键“在终端打开”优先执行的终端路径；配置不存在、读取失败、键不存在或键值为空时使用默认硬编码行为。
- 持久化数据：暂无。
...
| 外部命令 | shell 注入、缺命令降级、参数传递、`.desktop` Exec 兼容性、用户配置终端路径不可用 | 终端启动和文件打开均直接传 argv 调用；配置终端路径不经 shell，启动失败回到状态消息；文件打开当前覆盖 Exec 解析纯函数和编译路径，真实 GUI 应用启动需人工确认 | docs/dev/2-plan-context-menu-file-ops-properties.md；docs/dev/8-summary-selected-file-context-menu.md；docs/dev/21-task-filesystem-ini-config.md |
...
- 安装/部署方式：未定义。
- 配置变更：可选 `filesystem.ini` 放在可执行文件同级目录，当前支持 `name` 和 `terminal`。
- 升级步骤：未定义。
