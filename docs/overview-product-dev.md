...
> - 本次更新来源：docs/dev/46-task-filesystem-hook-mode.md、docs/dev/47-fix-yunbox-wps-presentation-open.md、docs/dev/48-fix-yunbox-wps-spreadsheets-open.md
...
- 模块划分：
  ...
  - `crates/filesystem-gui/src/tasks.rs`：目录加载、当前文件夹事件监听、文件名搜索、新建、XDG 模板目录读取、终端启动、应用注册表加载、用户级默认应用写入、外部应用启动和窗口 task 等后台/系统适配函数；阻塞型 I/O 继续通过 `Task::perform` 发起。
  - `crates/filesystem-gui/src/apps.rs`：本地 `.desktop`、通用和桌面专用 `mimeapps.list`、用户级默认应用写入、MIME 应用候选排序、文本编辑器兜底和 Exec field code 解析；直接以 argv 启动外部应用，不调用桌面服务。
...
- 启动配置规则：GUI 启动时默认通过 `current_exe()` 定位可执行文件同级 `filesystem.ini`；传入命令行参数 `--no-config`，或设置环境变量 `FILESYSTEM_NO_CONFIG` 为 `1`、`true`、`yes`、`on` 时直接使用 `RuntimeConfig::default()` 并跳过配置文件读取；设置环境变量 `FILESYSTEM_HIDE_TERMINAL_ENTRY` 为 `1`、`true`、`yes`、`on` 时写入 `RuntimeConfig.hide_terminal_entry`，即使同时禁用配置文件读取也生效；未禁用时解析顶层或 `[window]` section 下的非空 `name` 和 `terminal` 键，以及 `[blank-menu.*]` section 下的 `label`、`command` 和重复 `arg`；`name` 写入 `RuntimeConfig` 后覆盖左侧标题栏显示，`terminal` 写入 `RuntimeConfig` 后作为右键“在终端打开”的优先终端路径，合法 blank-menu section 以配置文件顺序写入空白菜单自定义命令列表；命令行 `--root <dir>` 由 `RootScope` canonicalize 为逻辑根，启动 cwd、根目录按钮和 Home fallback 均限制到该根下，访问入口通过 canonical path `starts_with(root)` 判定，目录导航越界时重新加载 root；普通模式侧边栏基础入口为 Home+Root，并加载 XDG 家目录快捷入口，root 模式侧边栏基础入口只保留 Root 且不保留 XDG 家目录快捷入口；配置文件不存在、读取失败、键为空或配置读取被禁用时使用默认值。
...
- GUI 右键菜单规则：`BrowserRightPressed` 同时判断 `selected_paths` 是否为空和指针是否命中条目；当前无选中项时，条目右键等价于空白处右键；空白菜单默认显示新建文件、新建文件夹、粘贴、全选、在终端打开和属性；检测到 XDG 模板目录直接普通文件时，空白菜单在新建文件夹后显示“新建文档”项，点击后右侧展开模板子菜单，选择模板会复制到当前目录并进入内联重命名；`RuntimeConfig.blank_menu_commands` 默认按配置顺序插入到“在终端打开”后的分割线与“属性”前的分割线之间；`RuntimeConfig.hide_terminal_entry` 为 true 时，空白处和文件夹菜单隐藏“在终端打开”，并在菜单高度和实际渲染中同时去掉对应分割线，空白菜单自定义命令仍在“全选”后的分组与“属性”前分组之间显示；单个文件夹已选中且右键命中该文件夹时显示文件夹菜单，默认支持打开、复制、剪切、重命名、删除、在终端打开和属性；单个非目录条目已选中且右键命中该条目时显示文件菜单，支持默认应用打开、打开方式、复制、剪切、重命名、删除和属性；多个路径已选中时显示多选菜单，只提供复制、剪切和删除；`context_menu_overlay` 使用主文件区宽度钳制菜单 x 坐标并保持固定宽度，按菜单项、分割线和内边距估算菜单高度并在浏览区域内钳制 y 坐标，模板子菜单展开时按主菜单加子菜单总宽度和主/子菜单较大高度钳制；菜单项内容水平左对齐、垂直居中；右键菜单面板使用实色背景和边框，不使用模糊阴影。
...
| `detect_path` / `detect_name` / `detect_bytes` | `filesystem-gui` | `filesystem-mime` | `detect_path` 最多读取文件前 256 KiB；内置规则优先，系统 shared-mime-info `magic`/`globs2` 只读 fallback；系统 magic 返回 `application/octet-stream` 等泛型结果时继续回落到内置名称/扩展名规则；不调用外部命令 | 支持文件图标、默认打开方式、打开方式列表和属性类型标签 |
| `load_app_registry()` | `filesystem-gui` | `filesystem-gui::apps` | 只读取本地 `XDG_CONFIG_HOME`/`~/.config`、`XDG_CONFIG_DIRS`、`XDG_DATA_HOME/applications`、`XDG_DATA_DIRS/applications` 下的 `.desktop`、桌面专用和通用 mimeapps 文件；trim `MimeType` 项；记录 `TextEditor` 分类；将 `[Default Applications]` 合并到 app 支持 MIME，允许无 `MimeType` 的默认应用成为候选；只接受通用 `mimeapps.list` 的 `[Added Associations]`/`[Removed Associations]` 影响候选；当前过滤 `Terminal=true` 应用；不调用服务 | 支持默认应用和打开方式列表 |
| `set_default_app_for_mime(mime, app_id)` | `filesystem-gui` | `filesystem-gui::apps` | 只写用户级 MIME apps 配置；有 `XDG_CURRENT_DESKTOP` 时写 `$XDG_CONFIG_HOME/<desktop>-mimeapps.list` 或 `$HOME/.config/<desktop>-mimeapps.list` 的 `[Default Applications]`，同时写通用 `mimeapps.list` 的 `[Added Associations]`；没有桌面名时在通用 `mimeapps.list` 更新两者；新应用放到该 MIME 应用列表首位并保留其它已有应用；不写系统目录、不调用外部命令 | 支持打开方式中设为默认打开方式 |
| `open_file_with_app(path, app)` | `filesystem-gui` | `filesystem-gui::apps` | 解析 `.desktop` Exec field code 后直接 `Command::spawn`，不经 shell；`%%` 保留字面 `%`，`%f/%F` 使用本地路径，`%u/%U` 默认使用 `file://` URI，WPS Office 本地文件兼容使用本地路径，WPS Writer/表格/演示的 `wps`/`et`/`wpp` 启动器默认优先尝试 `wpsoffice /prometheus <path>` 并保留原 `.desktop` 命令 fallback；`FILESYSTEM_HOOK_MODE=yunbox` 且应用为 WPS Presentation 时优先尝试 `office6/wpp <path>`，应用为 WPS Spreadsheets 时优先尝试 `office6/et <path>`，并保留原 `.desktop` 命令 fallback，未知或废弃字段码移除；当前工作目录设为文件父目录；启动成功后后台线程 wait 子进程 | 支持普通文件默认应用打开和打开方式打开 |
...
- 核心数据结构：
  ...
  - `TemplateFile`：空白菜单“新建文档”子菜单使用的模板文件显示名和源路径；显示名来自模板路径 `file_stem()`，因此菜单不显示最后一个扩展名，创建时仍使用源路径原始文件名。
  ...
  - `AppRegistry`/`DesktopApp`/`OpenWithDialog`/`DefaultAppAssignment`/`OpenFileOutcome`：本地应用列表、默认应用映射、桌面应用 Exec/MIME/Icon/TextEditor 分类声明、打开方式弹窗状态、默认应用写入结果和打开文件任务结果。
  - `ScanOptions`：当前仅包含 `show_hidden`。
- 配置文件/参数：启动时默认读取可执行文件同级 `filesystem.ini`；支持命令行参数 `--no-config` 和环境变量 `FILESYSTEM_NO_CONFIG=1/true/yes/on` 禁用该配置文件读取；支持环境变量 `FILESYSTEM_HIDE_TERMINAL_ENTRY=1/true/yes/on` 隐藏空白处和文件夹右键菜单中的“在终端打开”入口；支持环境变量 `FILESYSTEM_HOOK_MODE=sandbox|yunbox` 控制打开外部应用时的 hook 注入，默认清理继承的 `LD_PRELOAD`，`sandbox` 只注入 `/usr/local/andsec/sandbox/hook/hook-connect.so`，`yunbox` 只注入非空 `BOXFlLESO` 指向的云盒 hook；支持顶层或 `[window]` section 下的 INI 键 `name=xxx` 覆盖左侧标题栏显示，`terminal=xxx` 配置右键“在终端打开”优先执行的终端路径；支持 `[blank-menu.<id>]` section 为空白处右键菜单配置自定义项，其中非空 `label` 为菜单显示文本、非空 `command` 为可执行命令，多个 `arg=xxx` 按顺序作为 argv 参数传入，参数中的 `{cwd}` 替换为当前目录；支持命令行 `--root <dir>` 设置逻辑根目录，传入目录必须存在，所有目录访问和文件打开入口必须限制在 canonical root 下，root 模式侧边栏只显示根目录入口；只读解析 `$XDG_CONFIG_HOME/user-dirs.dirs` 或 `$HOME/.config/user-dirs.dirs` 的 `XDG_TEMPLATES_DIR` 来发现模板目录，并兼容 `$HOME/Templates`、`$HOME/Template`、`$HOME/模板`；配置不存在、读取失败、键不存在、键值为空或配置读取被禁用时使用默认硬编码行为。
- 持久化数据：用户显式勾选“设为默认打开方式”后，只写用户级 MIME apps 配置；有桌面名时桌面专用文件保存 `[Default Applications]`、通用 `mimeapps.list` 保存 `[Added Associations]`，无桌面名时通用文件保存两者；应用自身不维护额外数据库。
...
| 外部命令 | shell 注入、缺命令降级、参数传递、`.desktop` Exec 兼容性、用户配置终端路径或空白菜单自定义命令不可用 | 终端启动、空白菜单自定义命令和文件打开均直接传 argv 调用；配置终端路径和自定义命令不经 shell，启动失败回到状态消息；自定义命令参数只替换 `{cwd}`，不做 shell 拆分；文件打开当前覆盖 Exec 解析纯函数、后台 wait 子进程和编译路径，真实 GUI 应用启动需人工确认 | docs/dev/2-plan-context-menu-file-ops-properties.md；docs/dev/8-summary-selected-file-context-menu.md；docs/dev/21-task-filesystem-ini-config.md；docs/dev/22-task-blank-menu-custom-commands.md |
| 文件类型识别 | 内容签名误判、系统 shared-mime-info 数据差异、大目录大量文件前缀读取成本 | MIME 单测覆盖内置名称/扩展名、WPS 原生扩展名、远端 WPS 自有二进制头遇到泛型 magic 时保留扩展名 MIME、内容签名优先、zip Office、文本内容、`globs2` 和 magic 解析；内容读取上限 256 KiB 并在后台装饰任务执行；复杂系统 magic 子规则保守跳过 | docs/dev/9-summary-filesystem-mime.md；docs/dev/25-fix-wps-docx-open.md |
| 本地应用/默认应用解析与写入 | 不同发行版 `.desktop`、通用/桌面专用 `mimeapps.list`、`Terminal=true`、MIME 声明和用户配置权限差异 | GUI 单测覆盖 `text/plain` 应用匹配文本类 MIME、`TextEditor` 分类兜底、未知二进制不走文本编辑器、WPS/标准 Office MIME 互认、WPS `.desktop` MIME 缺失兜底、无 `MimeType` 默认应用、mimeapps Added/Removed Associations 合并和优先级、desktop-specific Added/Removed 忽略、默认应用与关联拆分写入、`Terminal=true` 过滤、Exec field code、`%u/%U` file URI、WPS `%u/%U` 本地路径兼容、WPS Writer/表格/演示 `wpsoffice /prometheus` 首选命令和 `.desktop` fallback、云盒 WPS Presentation `office6/wpp` 首选命令和 `.desktop` fallback、云盒 WPS Spreadsheets `office6/et` 首选命令和 `.desktop` fallback、未知/废弃字段码移除、`%%` 字面百分号、嵌套 applications 目录、`XDG_CURRENT_DESKTOP` 桌面名拆分、桌面专用 mimeapps 优先级、用户级 mimeapps section 补齐、默认应用置顶和进程内默认映射同步；未知类型使用保守回退 | docs/dev/8-summary-selected-file-context-menu.md；docs/dev/9-summary-filesystem-mime.md；docs/dev/11-fix-text-editor-fallback.md；docs/dev/17-fix-desktop-exec-field-codes.md；docs/dev/24-task-open-with-default-app.md；docs/dev/25-fix-wps-docx-open.md；docs/dev/26-fix-wps-sandbox-prometheus-open.md；docs/dev/47-fix-yunbox-wps-presentation-open.md；docs/dev/48-fix-yunbox-wps-spreadsheets-open.md |
...
- 关键任务文档：
  ...
  - docs/dev/23-task-current-folder-auto-refresh.md
  - docs/dev/24-task-open-with-default-app.md
...
| 2026-09-14 | 记录 `--no-config` 和 `FILESYSTEM_NO_CONFIG` 跳过 `filesystem.ini` 读取的启动配置控制流 | 更新 GUI 启动配置规则 | docs/dev/43-task-disable-runtime-config.md |
| 2026-09-14 | root 模式侧边栏基础入口只保留 Root，普通模式继续显示 Home+Root 和 XDG 家目录快捷入口 | 更新 GUI 侧边栏启动参数分支 | docs/dev/44-task-root-sidebar-only-root.md |
| 2026-09-14 | 记录 `FILESYSTEM_HIDE_TERMINAL_ENTRY` 隐藏右键菜单终端入口并同步调整菜单分割线和高度 | 更新 GUI 右键菜单和运行时配置规则 | docs/dev/45-task-hide-terminal-menu-entry.md |
| 2026-09-15 | 记录 `FILESYSTEM_HOOK_MODE=sandbox|yunbox` 控制外部应用 hook 注入，区分沙盒 hook 与云盒 hook | 更新 GUI 外部应用启动环境规则 | docs/dev/46-task-filesystem-hook-mode.md |
| 2026-09-16 | 记录 `FILESYSTEM_HOOK_MODE=yunbox` 下 WPS Presentation 优先尝试 `office6/wpp`，Writer/表格继续使用 Prometheus 入口 | 更新外部应用启动兼容规则 | docs/dev/47-fix-yunbox-wps-presentation-open.md |
| 2026-09-16 | 记录 `FILESYSTEM_HOOK_MODE=yunbox` 下 WPS Spreadsheets 优先尝试 `office6/et`，Writer 继续使用 Prometheus 入口 | 更新外部应用启动兼容规则 | docs/dev/48-fix-yunbox-wps-spreadsheets-open.md |
