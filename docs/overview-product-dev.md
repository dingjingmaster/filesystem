...
- 配置文件/参数：启动时读取可执行文件同级 `filesystem.ini`；支持顶层或 `[window]` section 下的 INI 键 `name=xxx` 覆盖左侧标题栏显示，`terminal=xxx` 配置右键“在终端打开”优先执行的终端路径；支持 `[blank-menu.<id>]` section 为空白处右键菜单配置自定义项，其中非空 `label` 为菜单显示文本、非空 `command` 为可执行命令，多个 `arg=xxx` 按顺序作为 argv 参数传入，参数中的 `{cwd}` 替换为当前目录；支持命令行 `--root <dir>` 设置逻辑根目录，传入目录必须存在，所有目录访问和文件打开入口必须限制在 canonical root 下，root 模式隐藏家目录常见快捷入口；只读解析 `$XDG_CONFIG_HOME/user-dirs.dirs` 或 `$HOME/.config/user-dirs.dirs` 的 `XDG_TEMPLATES_DIR` 来发现模板目录，并兼容 `$HOME/Templates`、`$HOME/Template`、`$HOME/模板`；配置不存在、读取失败、键不存在或键值为空时使用默认硬编码行为。
