...
| `set_default_app_for_mime(mime, app_id)` | `filesystem-gui` | `filesystem-gui::apps` | 只写用户级 MIME apps 配置；有 `XDG_CURRENT_DESKTOP` 时优先写 `$XDG_CONFIG_HOME/<desktop>-mimeapps.list` 或 `$HOME/.config/<desktop>-mimeapps.list`，否则写通用 `mimeapps.list`；补齐 `[Default Applications]` 和 `[Added Associations]`；新应用放到该 MIME 应用列表首位并保留其它已有应用；不写系统目录、不调用外部命令 | 支持打开方式中设为默认打开方式 |
...
- 持久化数据：用户显式勾选“设为默认打开方式”后，写入当前桌面优先的用户级 MIME apps 配置文件；应用自身不维护额外数据库。
