...
- 模块划分：
  ...
  - `crates/filesystem-gui/src/icons.rs`：主题图标解析、MIME 到主题图标名映射和本地 SVG 回退策略，封装 `IconResolver`。
...
| `IconResolver` | `filesystem-gui` 后台任务 | 本地图标主题文件 | 只查找并读取本地 SVG 图标文件，不调用桌面服务；找不到就回退本地 `icons/file.svg` | 根据 MIME 类型映射常见主题图标名，完成消息携带已读入内存的 SVG |
...
- 核心数据结构：
  ...
  - `DisplayEntry`/`EntryIcon`：GUI 层显示条目、缓存 MIME 和图标来源，图标来源为编译期 SVG 或后台读入内存的本地图标主题 SVG。
