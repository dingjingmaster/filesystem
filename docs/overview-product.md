...
- 核心功能：本地目录浏览、隐藏文件过滤、无系统边框窗口、窗口拖拽/关闭/最小化/最大化/边缘缩放、侧边栏本地导航、可编辑地址栏、网格文件图标、基础文件类型展示；后续逐步补充安全写操作和外部二进制适配。
...
| 场景 | 用户目标 | 成功标准 | 异常/边界 |
|------|----------|----------|-----------|
| 浏览本地目录 | 查看当前目录内容并进入子目录 | GUI 以暗色文件管理器布局展示目录条目，目录优先排序，可进入子目录和返回上级 | 路径不存在或不可读时展示错误状态 |
...
- 用户可见行为：默认不显示隐藏文件；普通文件暂不自动打开；侧边栏只显示主文件夹、根目录和用户家目录中实际存在的常见目录；侧边栏顶部和地址栏右侧空白区可拖拽窗口；窗口四边和四角可拖动缩放；地址栏可编辑并回车访问路径。
