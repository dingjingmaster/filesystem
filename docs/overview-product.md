...
- 启动配置规则：启动时默认检查可执行文件同级目录的 `filesystem.ini`；传入命令行参数 `--no-config`，或设置环境变量 `FILESYSTEM_NO_CONFIG` 为 `1`、`true`、`yes`、`on` 时跳过读取该文件并使用默认启动配置；该 INI 文件中非空 `name=xxx` 会覆盖左侧拖拽标题栏显示，非空 `terminal=xxx` 会让“在终端打开”执行该终端路径并把当前目录或目标文件夹设为进程当前工作目录；`[blank-menu.*]` section 可通过 `label`、`command` 和多个 `arg` 为空白处右键菜单新增自定义命令，参数中的 `{cwd}` 会替换为当前目录；命令行 `--root <dir>` 会把已存在目录作为逻辑根，启动后打开该目录，根目录按钮指向该目录，目录访问、地址栏绝对路径、历史导航、软链接目标和打开文件入口均不得跳出该目录，目录越界导航会自动回到该目录，且侧边栏隐藏下载、图片、桌面、文档、音乐、视频等家目录快捷入口；未配置或禁用配置读取时保持默认左侧标题 `文件` 和 `terminator`、`mate-terminal`、`gnome-terminal` PATH fallback，且不显示额外空白菜单项。
