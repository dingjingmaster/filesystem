...
- 打开方式列表规则：每个 APP 行左侧显示 APP 图标和名称，内容水平左对齐、垂直居中；选中对钩水平右对齐；默认应用排在最前，若没有默认应用则按精确 MIME、`text/plain` 文本兜底、`TextEditor` 分类兜底、类型通配、全局通配和名称排序；勾选“设为默认打开方式”后确认打开，会把所选应用写入当前桌面优先的用户级 MIME apps 配置文件的 `[Default Applications]` 和 `[Added Associations]`，并放到该 MIME 应用列表首位；APP 图标按 `.desktop` 的 `Icon` 字段查找本地图标，找不到时使用 `icons/app.svg`。
